...
Entry point is `server.py`: it owns the HTTP server, the job system, API routes under `/api/*`, and static file serving.
...
```
GET  /api/config                    — returns server config (e.g. debug flag)
GET  /api/jobs/stream               — SSE stream (see below)
POST /api/jobs/start                — body: {"job_type": "scan-media"}
POST /api/jobs/{job_id}/pause
POST /api/jobs/{job_id}/resume
POST /api/jobs/{job_id}/abort
POST /api/jobs/{job_id}/dismiss     — removes a failed job from the registry
DELETE /api/debug/csv               — deletes media.csv (debug only)
```
